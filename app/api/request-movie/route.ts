...
console.log("[v0] API Route loaded - request-movie endpoint using MongoDB at", new Date().toISOString())
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!body.movieId || !body.title) {
      return NextResponse.json({ error: "Missing required fields: movieId and title are required" }, { status: 400 })
    }
    ...
    if (existing) {
      return NextResponse.json({ error: "Already requested", request: existing }, { status: 400 })
    }
    ...
    console.log("[v0] Movie request created successfully:", newRequest.title, "ID:", newRequest._id)

    try {
      const realTimeManager = RealTimeManager.getInstance()
      await realTimeManager.triggerUpdate()
      console.log("[v0] Real-time notification sent to admin dashboard")
    } catch (notificationError) {
      console.error("[v0] Failed to send real-time notification:", notificationError)
    ...
    return NextResponse.json({ error: "Failed to process movie request. Please try again." }, { status: 500 })
  }
}
...
export async function GET() {
  ...
  try {
    ...
    console.log("[v0] Found", requests.length, "requests")
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    const request = await MovieRequest.findByIdAndUpdate(body._id || body.id, { ...body }, { new: true })
    ...
    try {
      const realTimeManager = RealTimeManager.getInstance()
      await realTimeManager.triggerUpdate()
      console.log("[v0] Real-time update sent for status change:", request.status)
    } catch (notificationError) {
      console.error("[v0] Failed to send real-time notification:", notificationError)
    }
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    try {
      const realTimeManager = RealTimeManager.getInstance()
      await realTimeManager.triggerUpdate()
      console.log("[v0] Real-time update sent for request deletion")
    } catch (notificationError) {
      console.error("[v0] Failed to send real-time notification:", notificationError)
    }
