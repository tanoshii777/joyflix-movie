...
export default function RecommendationEngine({
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/recommendations?watchedMovies=${encodeURIComponent(
  ...
  const getLocalRecommendations = () => {
    const viewingHistory = JSON.parse(
      localStorage.getItem("viewingHistory") || "[]"
    );
    ...
    const watchedCategories = [
      ...new Set(watchedMovies.map((movie) => movie.category)),
    ];
    ...
    const categoryRecommendations = movies
      .filter(
        (movie) =>
          !viewingHistory.includes(movie.id) &&
          watchedCategories.includes(movie.category)
      )
      .slice(0, 8);
    ...
    const recentMovies = movies
      .filter(
        (movie) =>
          !viewingHistory.includes(movie.id) && movie.year && movie.year >= 2020
      )
      .slice(0, 6);
    ...
    return {
      ...
      hybrid: [
        ...categoryRecommendations.slice(0, 6),
        ...recentMovies.slice(0, 6),
      ].slice(0, 12),
    };
  };
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-2">
          {userStats && (
            <div className="hidden sm:block text-xs text-muted-foreground mr-4">
              {userStats.totalRatings} ratings • Avg:{" "}
              {userStats.averageRating.toFixed(1)}/10
            </div>
          )}

          <div className="flex items-center justify-between sm:justify-start">
            <div className="flex gap-2 sm:gap-1 flex-1 sm:flex-initial">
              <Button
                variant={activeType === "hybrid" ? "default" : "ghost"}
                size="sm"
                onClick={() => setActiveType("hybrid")}
                className="text-sm sm:text-xs px-3 sm:px-2 py-2 sm:py-1 flex-1 sm:flex-initial min-w-0"
              >
                For You
              </Button>
              <Button
      ...
      {loading ? (
        <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="w-36 sm:w-40 h-52 sm:h-60 bg-gray-800 rounded-lg animate-pulse flex-shrink-0"
            />
          ))}
        </div>
      ) : (
        <div className="movie-grid-mobile sm:overflow-visible sm:grid sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 scrollbar-hide">
